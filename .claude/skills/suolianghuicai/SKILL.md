...
| 不要 | 原因 | 正确做法 |
|---|---|---|
| 给策略 3 显式传 `--volume-ratio 0.6` | 脚本用"是否等于 0.6"判断是否走默认，传 0.6 会被误判为"未传"而强改 0.7 | 策略 3 不传该参数（用默认 0.7），或传 0.7 以外的值 |
...
### 参数一致性说明
策略 3 成交量阈值：**策略描述（涨停日成交量的 70%）与默认参数（0.7）一致**，以 0.7 为准。（`analyzer.py` 内部 docstring 仍写 0.5 属代码遗留，不影响实际运行。）
