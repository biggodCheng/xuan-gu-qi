...
### 策略3：单日缩量回踩
涨停日后，某天成交量低于涨停日成交量的70%，且收盘价低于涨停日收盘价。
适合捕捉快速缩量回调的股票。
...
3. 可选参数：
   ...
   - `--volume-ratio 0.6`（策略2/3：成交量比例，策略2默认0.6，策略3默认0.7）
...
| 不要 | 原因 | 正确做法 |
|---|---|---|
| 仅凭 `current_close < last_zt_close` 就认定在回踩 | `current_close` 取 K 线最后一天，可能与回踩段脱节 | 结合 `pullback_days` 和 `volume_shrink_ratio` 综合判断 |
| 期望三策略结果可复现 | 三策略逻辑不同，同一股票可能只在某策略命中 | 按策略分别看，命中即关注 |
