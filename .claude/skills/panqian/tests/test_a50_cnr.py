...
- A50(hf_CHA50CFD)可达,15 字段:[0]价 [3]昨收 → parser 自算 pct = (price-prev)/prev*100。
...
def test_parse_a50_cnr_fixture_a50_present_cnr_present():
    ...
    # price=[0]=15165.200, prev=[3]=15166.000(fixture)
    assert d["a50"]["price"] == pytest.approx(15165.200, abs=1e-9)
    # pct 自算:(price-prev)/prev*100 — 数学验证 IDX_PRICE_HF=0 / IDX_PREV_HF=3
    expected_pct = (15165.200 - 15166.000) / 15166.000 * 100
...
def test_pick_hf_prev_zero_returns_none():
    # prev<=0 应返回 None(防除零)
    fields = ["100", "", "100", "0", "110", "90", "22:00:00", "", "", "", "", "", "2026-07-23", "名", "0"]
    assert a50_cnr._pick_hf(fields) is None


def test_pick_hf_malformed_returns_none():
    # 非数字 price/prev 应返回 None(不抛)
    bad = ["NOTANUM", "", "100", "100", "110", "90"]
